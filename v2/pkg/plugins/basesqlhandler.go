...
type SqlBackend interface {
	...
	CreateSchema(ctx context.Context, db *sql.DB)
	// Migrate schema if necessary
	MigrateSchema(ctx context.Context, db *sql.DB, checker func(context.Context, *sql.DB, string, string) bool)
	//
...
func NewDatabaseHandler(sqlBackend SqlBackend, opts ...handler.Option) handler.Handler {
	...
	sqlBackend.CreateSchema(ctx, db)
	sqlBackend.MigrateSchema(ctx, db, ColumnExists)
...
func ColumnExists(ctx context.Context, db *sql.DB, tableName string, columnName string) bool {
	var found string
